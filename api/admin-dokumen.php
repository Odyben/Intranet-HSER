...
requireAdmin();
if ($_SERVER['REQUEST_METHOD'] !== 'GET') requireModuleAccess('dokumen');
$conn = getDbConnection();

header('Content-Type: application/json');
$method = $_SERVER['REQUEST_METHOD'];

$uploadDir = __DIR__ . '/../Dokumen/uploads/dokumen/';
$uploadUrlPrefix = 'Dokumen/uploads/dokumen/';
...
switch ($method) {
    case 'GET':
        ...
        $countSql = "SELECT COUNT(*) as total FROM dokumen WHERE 1=1";
        $dataSql = "SELECT * FROM dokumen WHERE 1=1";
        $params = [];
        $types = "";
    ...
    case 'POST':
        // Multipart form: creates a new document, or updates an existing one
        // when "id" is present. A new file is optional on update — if omitted,
        // the existing file_path/type/size are kept as-is.
        ...
        if ($hasNewFile) {
            ...
            $type = $ext;
            $size = formatDocFileSize($_FILES['file']['size']);
        } elseif (!$id) {
        ...
        if ($id) {
            ...
            // when one was.
            $existing = $conn->prepare("SELECT file_path, type, size FROM dokumen WHERE id = ?");
            ...
            $stmt = $conn->prepare("UPDATE dokumen SET name=?, category=?, cat_label=?, version=?, doc_date=?, size=?, type=?, uploader=?, description=?, file_path=? WHERE id=?");
            ...
            if ($stmt->execute()) {
                ...
                logAudit($conn, 'update', 'dokumen', 'Mengemaskini dokumen: ' . $name);
                echo json_encode(["success" => true, "message" => "Dokumen berjaya dikemaskini"]);
            } else {
            ...
            if ($stmt->execute()) {
                logAudit($conn, 'create', 'dokumen', 'Menambah dokumen: ' . $name);
                echo json_encode(["success" => true, "id" => $stmt->insert_id, "message" => "Dokumen berjaya ditambah"]);
            } else {
                @unlink($destPath);
                echo json_encode(["error" => $stmt->error]);
            }
    ...
    case 'DELETE':
        ...
        $stmt = $conn->prepare("SELECT name, file_path FROM dokumen WHERE id = ?");
        ...
        if ($stmt->execute()) {
            ...
            // (kept in Dokumen/uploads/dokumen/) — never touch the original
            // manually-placed files elsewhere in Dokumen/.
            if ($row && strpos($row['file_path'], $uploadUrlPrefix) === 0) {
                $filePath = __DIR__ . '/../' . $row['file_path'];
                if (file_exists($filePath)) @unlink($filePath);
            }
            logAudit($conn, 'delete', 'dokumen', 'Memadam dokumen: ' . ($row['name'] ?? ('ID ' . $id)));
            echo json_encode(["success" => true, "message" => "Dokumen berjaya dipadam"]);
        } else {
            echo json_encode(["error" => $stmt->error]);
        }
        $stmt->close();
